...
const AddDoctor = () => {
  ...
  const submitHandler=async (event) => {
    ...
    try {
      if(!docImg){
        return toast.error("Image not inserted")
      }
    ...
    <form onSubmit={submitHandler} className=' m-5 w-full'>
      ...
      <div className=' bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        <div className='flex items-center gap-4 mb-8 text-gray-500'>
          ...
          <input onChange={(e)=>setDocImg(e.target.files[0])} type="file" id="doc-img" className=' cursor-pointer' hidden/>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
          ...
          <div className='w-full lg:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex-1 flex flex-col gap-1'>
              ...
              <input onChange={(e)=>setAddress2(e.target.value)} value={address2} className=' border rounded px-3 py-2' type="text" placeholder='Address 1' />
            </div>
        ...
        <button className=' cursor-pointer bg-primary text-white px-10 py-3 mt-4 rounded-full'>Add Doctor</button>
      </div>
    </form>
  )
}
